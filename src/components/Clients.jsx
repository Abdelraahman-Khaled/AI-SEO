        ...
        <div className="flex overflow-hidden mt-10 [mask-image:linear-gradient(to_right,transparent,black_20%,black_80%,transparent)]">
          ...
          >
            {[...clients, ...clients].map((client, index) => (
              <div
                key={index}
                className="flex-none border border-white/15 p-6 md:p-10 rounded-xl bg-[linear-gradient(to_bottom_left,rgb(140,69,255,0.3),black)] max-w-xs md:max-w-md "
              >
                ...
                <div className="flex items-center gap-3 mt-5">
                  ...
                  <div>
                    <div>{client.name}</div>
                    <div className="text-white/50 text-sm">{client.title}</div>
                  </div>
                </div>
              </div>
            ))}
